...
void PinwheelApp::app_render_frame(dvec2 screen_size, double delta)  {
  ...
  uint32_t insn_a = b24(pinwheel.core.A_hpc_.pc) ? uint32_t(pinwheel.code_ram.get()) : 0;

  auto thread_a = pinwheel.core.A_hpc_.hart;
  auto thread_b = pinwheel.core.B_hpc_.hart;
  auto thread_c = pinwheel.core.C_hpc_.hart;
  //auto thread_d = pinwheel.core.D_hpc.hart;

  uint32_t thread_pcs[16] = {0};
  //if (pinwheel.core.D_pc) thread_pcs[thread_d] = pinwheel.core.D_pc;
  if (pinwheel.core.C_hpc_.pc) thread_pcs[thread_c] = pinwheel.core.C_hpc_.pc;
  if (pinwheel.core.B_hpc_.pc) thread_pcs[thread_b] = pinwheel.core.B_hpc_.pc;
  if (pinwheel.core.A_hpc_.pc) thread_pcs[thread_a] = pinwheel.core.A_hpc_.pc;

  auto thread_a_col = pinwheel.core.A_hpc_.pc ? thread_a + 2 : 0;
  auto thread_b_col = pinwheel.core.B_hpc_.pc ? thread_b + 2 : 0;
...

  {
    ...
    d("pc   0x%08x\n", pinwheel.core.B_hpc_.pc);
    d("op   0x%08x ",  pinwheel.core.B_insn_.raw); print_rv(d, pinwheel.core.B_insn_.raw); d("\n");
    d("r1   0x%08x\n", pinwheel.regs.get_rs1());
    d("r2   0x%08x\n", pinwheel.regs.get_rs2());
    const auto imm_b  = pinwheel.core.dbg_decode_imm(pinwheel.core.B_insn_);
...

  {
    ...
    //d("pc   0x%08x\n", pinwheel.core.C_pc);
    d("op   0x%08x ",  pinwheel.core.C_insn_.raw); print_rv(d, pinwheel.core.C_insn_.raw); d("\n");
    d("addr 0x%08x\n", pinwheel.core.C_addr_);
    d("res  0x%08x\n", pinwheel.core.C_result_);
  ...
  */
  ...
  code_painter.highlight_x = ((b24(pinwheel.core.B_hpc_.pc) & 0xFFFF) >> 2) % 16;
  code_painter.highlight_y = ((b24(pinwheel.core.B_hpc_.pc) & 0xFFFF) >> 2) / 16;
  ...
  console_painter.dump(view, screen_size, 32*19,  32, 1, 1, pinwheel_tb.console1.width, pinwheel_tb.console1.height, vec4(0.0, 0.0, 0.0, 0.4), (uint8_t*)pinwheel_tb.console1.buf_);
  console_painter.dump(view, screen_size, 32*19, 256, 1, 1, pinwheel_tb.console2.width, pinwheel_tb.console2.height, vec4(0.0, 0.0, 0.0, 0.4), (uint8_t*)pinwheel_tb.console2.buf_);
  console_painter.dump(view, screen_size, 32*19, 480, 1, 1, pinwheel_tb.console3.width, pinwheel_tb.console3.height, vec4(0.0, 0.0, 0.0, 0.4), (uint8_t*)pinwheel_tb.console3.buf_);
  console_painter.dump(view, screen_size, 32*19, 704, 1, 1, pinwheel_tb.console4.width, pinwheel_tb.console4.height, vec4(0.0, 0.0, 0.0, 0.4), (uint8_t*)pinwheel_tb.console4.buf_);

  text_painter.render_string(view, screen_size, "UART", 1024 + 256, 704 + 128 + 32 - 12);
  console_painter.dump(view, screen_size, 1024 + 256, 704 + 128 + 32, 1, 1, pinwheel_tb.console5.width, pinwheel_tb.console5.height, vec4(0.0, 0.0, 0.0, 0.4), (uint8_t*)pinwheel_tb.console5.buf_);
